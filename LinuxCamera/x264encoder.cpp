...
int X264Encoder::InitH264(int width, int height, int fps, int bit_rate)
{
    width_ = width;
    height_ = height;
    fps_ = fps;
...
AVPacket *X264Encoder::Encode(uint8_t *yuv_data, int yuv_size, int stream_index, int64_t pts, int64_t time_base)
{
    if(!codec_ctx_)
    {
...
int X264Encoder::Encode(uint8_t *yuv_data, int yuv_size,
...
AVCodecContext *X264Encoder::GetCodecContext()
{
    return codec_ctx_;
}
